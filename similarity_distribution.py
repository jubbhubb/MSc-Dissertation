...
def similarity_histogram(experiment_name):
    filepath = (f"experiments/{experiment_name}")
    df = pd.read_csv(f"{filepath}/quote_explanation_semantic_validation.csv")
    ...
    plt.savefig(f"{filepath}/similarity_distribution.pdf", dpi=300)
    plt.show()
