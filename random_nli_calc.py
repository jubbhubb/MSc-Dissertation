...
def random_nli_calc(experiment_folder, test_name):
    ...
    """
    df = pd.read_csv(f"{experiment_folder}/statistics/nli_random_baseline.csv")

    labels = (df["type"] == "True").astype(int)

    auc = roc_auc_score(labels, df["entailment"])
    
    with open(f"{experiment_folder}/statistics/nli_random_baseline_stats.txt", "w") as f:
    ...
    with open(f"{experiment_folder}/statistics/nli_random_baseline_stats.txt", "a") as f:
