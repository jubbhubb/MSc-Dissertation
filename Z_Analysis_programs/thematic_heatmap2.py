...
def theme_similarity_heatmap(input_file, folder, display=False):
    ...
    similarity_df = pd.DataFrame(similarity_matrix, index=theme_names, columns=theme_names)
    ...
    for i in range(len(theme_names)):
        for j in range(i + 1, len(theme_names)):
            relationships.append((theme_names[i], theme_names[j], similarity_matrix[i, j]))
    ...
    sns.heatmap(similarity_df, annot=True, fmt=".2f", cmap="viridis", vmin=0, vmax=1)

    plt.title("Semantic Similarity Between LLM-Generated Themes")
    ...
    theme_centrality = similarity_df.apply(lambda row: (row.sum()-1)/(len(row)-1))
